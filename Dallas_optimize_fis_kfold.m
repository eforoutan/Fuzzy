...
for f = 1:num_folds
   ...
   %% Load FIS
    filename_fis = sprintf("Dallas_exp_tuned_fold_%d.fis", f);
...
%% Save the optimized FIS
    writeFIS(fis, sprintf('Optimized_Dallas_exp_tuned_fold_%d.fis', f));
...
writetable(results_table, 'Optimized_Dallas_EXP_Tuning_Evaluation_PS_PSO.csv'); 
tt = toc